...
function LimpiarPoppupPuerto() {
    ...
    $('#lblRegistro').html('Crear Tipo Contenedor');
}
...
function EditarPuerto(puerId) {
    if (puerId != null && puerId != '') {
        ...
        $('#lblRegistro').html('Actualizar Tipo Contenedor');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
		$('#txtNombre').focus();
        //ShowPopUp('divPopup');
    }
...
function EliminarPuerto(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar el tipo de contenedor "' + puerNombre + '"?', function (result) {
        if (result) {
