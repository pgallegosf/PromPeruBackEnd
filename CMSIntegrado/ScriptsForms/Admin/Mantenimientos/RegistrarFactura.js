...
function EditarFactura(usuaId) {
    if (usuaId != null && usuaId != '') {
        ...
        $('#lblRegistro').html('Actualizar factura');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
        //ShowPopUp('divPopup');
    }
}
...
function EliminarFactura(usuaNombre, usuaId) {
    confirm('¿Está seguro de eliminar la factura "' + usuaNombre + '"?', function (result) {
        if (result) {
            ...
            $.ajax({
                ...
                success: function (v) {
                    if (v == "true") {
                        alert('Se eliminó la factura "' + usuaNombre + '"', 'CHECK');
                ...
                error: function (v) {
                    alert('No se pudo eliminar la factura "' + usuaNombre + '"', 'ERROR');
                }
            });
        } else {
            return false;
