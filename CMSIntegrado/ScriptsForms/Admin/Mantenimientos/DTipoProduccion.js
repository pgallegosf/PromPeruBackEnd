...
function EditarFactura(usuaId) {
    if (usuaId != null && usuaId != '') {
        ...
        $('#lblRegistro').html('Actualizar Tipo Producción');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
        //ShowPopUp('divPopup');
    }
}
...
function EliminarFactura(usuaNombre, usuaId) {
    confirm('¿Está seguro de eliminar el tipo de producción "' + usuaNombre + '"?', function (result) {
        if (result) {
            ...
            $.ajax({
                ...
                success: function (v) {
                    if (v == "true") {
                        ...
                        alert('No se pudo eliminar el tipo de producción "' + usuaNombre + '" porque tiene dependencias', 'WARNING');
                },
                error: function (v) {
                    alert('No se pudo eliminar el tipo de producción "' + usuaNombre + '"', 'ERROR');
                }
            });
        } else {
            return false;
