...
$(document).ready(function () {
    ...
    $('#txtTiempo').keypress(function (event) {
        var Ffin = document.getElementById('txtTiempo');
        Ffin.maxLength = 5;
        return ValidarHora(event);
    });

    $('#txtTiempo').on('paste', function (e) {
        return false;
    });
...
function EditarPuerto(puerId) {
    if (puerId != null && puerId != '') {
        ...
        $('#lblRegistro').html('Actualizar Escala');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
        //ShowPopUp('divPopup');
    }
}
