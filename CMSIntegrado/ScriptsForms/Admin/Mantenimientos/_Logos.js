...
$(document).ready(function () {
	...
	const $seleccionArchivos2 = document.querySelector("#seleccionArchivo2"),
...
function InsertarPuerto() {
    ...
    alert('Se actualizaron los logos correctamente', 'CHECK', function () {
        LimpiarPoppupPuerto();
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
        HidePopUp('divPopup');
    });
...
function EliminarCarga(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar la ruta "' + puerNombre + '"?', function (result) {
        if (result) {
