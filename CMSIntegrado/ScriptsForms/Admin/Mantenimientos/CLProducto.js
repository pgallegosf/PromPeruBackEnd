//cargarPerfil();
var listaSinonimos = [];
	
$(document).ready(function () {
	listaSinonimos.push('oliva2');
    //ValidarAccesoGeneral();
    $('#txtNombre').focus();
    var $btnNuevoPuerto = $('#btnNuevo');
	var $btnNuevaPartida = $('#btnNuevaPartida');
    var $btnGuardarPuerto = $('#btnGuardar');
    var $btnListarPuerto = $('#btnListarPuerto');
    var $btnCancelar = $('#btnCancelar');
    var $idPuerto = $('#idPuerto');
    var $txtNombre = $('#txtNombre');
	var $btnAdicionarSinonimo = $('#btnAdicionarSinonimo');
	var $btnCancelarPartida = $('#btnCancelarPartida');

    $(document).keyup(function (event) {
        if (event.which == 27) {
            $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
            //ShowPopUp('divPopup');
        }
    });
	
	$btnNuevaPartida.click(function () {
        $('.reveal-modal-bg2').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
        //ListarPartidas();
    });
	$btnAdicionarSinonimo.click(function () {
        //AgregarSinonimo();
        AgregarFilaNueva();
    });
	$btnCancelarPartida.click(function () {
        $('.reveal-modal-bg2').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
    });
    $btnCancelar.click(function () {
        //HidePopUp('divPopup');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
    });

    $('#txtNombre').keypress(function (event) {
        var Ffin = document.getElementById('txtNombre');
        Ffin.maxLength = 25;
        return ValidarSoloLetras(event);
    });

    $('#txtNombre').on('paste', function (e) {
        return false;
    });

    

    $btnNuevoPuerto.click(function () {
        LimpiarPoppupPuerto();
        //ShowPopUp('divPopup'); 

        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });

        $('#txtNombre').focus();
    })

    $btnGuardarPuerto.click(function () {
        

        var num = 0;
        

        if (ValidarCamposPuerto() == false) {
            return;
        } 
        else if ($('#idPuerto').html() == '') {
            InsertarPuerto();
        } else {
            ActualizarPuerto();
        }
    })

    $('#txtNombre').keyup(function () {
        $('#puerto').html(1);
    });

});

function LimpiarPoppupPuerto() {
    $('#txtNombre').val('');
    $('#ddlPuerto').val(0);    
    $('#idPuerto').html('');
    $('#txtNombre').focus();
    $('#chkActivo').prop('checked', 'checked');
    $('#btnGuardar').html('<label>Registrar</label>');
	$('#lblRegistrar').html('Registrar');
    $('#lblRegistro').html('Crear Producto');
}

function InsertarPuerto() {
    ShowLoader();
    var puertoActivo;
    var Anulacion;
    var salida = false;
    var nombre = $('#txtNombre').val().trim();
    var pais = $('#ddlPais').val();

    if ($('#chkActivo').prop('checked')) {
        puertoActivo = true;
    }
    else {
        puertoActivo = false;
    }

    var nuevoPuerto = {
        Puer_Nombre: nombre,
        Pais_Id: pais,
        Puer_Activo: puertoActivo
    };

    alert('Se guardó el puerto "' + nombre + '" correctamente', 'CHECK', function () {
        LimpiarPoppupPuerto();
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
        HidePopUp('divPopup');
    });

    if (salida) {
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
        //HidePopUp('divPopup');
    }
    HideLoader();
}

function ActualizarPuerto() {
    ShowLoader();
    var puertoActivo;
    var salida = false;
    var nombre = $('#txtNombre').val();
    var pais = $('#ddlPais').val();

    if ($('#chkActivo').prop('checked')) {
        puertoActivo = true;
    }
    else {
        puertoActivo = false;
    }

    var puerto = $('#idPuerto').html();

    var ActualizarPuerto = {
        Puer_Id: puerto,
        Puer_Nombres: nombre,
        Pais_Id: pais
    };

    if (salida) {
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
        //HidePopUp('divPopup');
    }
}

function EditarPuerto(puerId) {
    if (puerId != null && puerId != '') {
        SeleccionarPuerto(puerId);
        $('#btnGuardar').html('<label>Actualizar</label>');
		$('#lblRegistrar').html('Actualizar');
        $('#lblRegistro').html('Actualizar Producto');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
		$('#txtNombre').focus();
        //ShowPopUp('divPopup');
    }
}

function AgregarSinonimo() {
    var gridSinonimos = document.getElementById('gridSinonimos');
    if (gridSinonimos.rows.length > 1) {
        listaSinonimos.splice(0, listaSinonimos.length);
        for (i = 1 ; i < gridSinonimos.rows.length ; i++) {
            var sinonimoNombre = $('#txtSinonimoNombre' + i).val();

            var sinonimo = "";
            sinonimo = {
                Sino_Id: i,
                Sino_Nombre: sinonimoNombre
            };

            listaSinonimos.push(sinonimo);
        }
    }

    LimpiarAgregarFilasTablaSinonimo();
}
function AgregarFilaNueva() {
	listaSinonimos.push("sinonimo nuevo");
    var html = "";
    //if ((listaSinonimos.length + 1)  % 2 == 0) {
    //    html += "<tr class='GrillaRow'>";
    //} else {
        html += "<tr class='GrillaRowAlternativo'>";
    //}
    html += "<td style='width:5%' align='center'>" + (listaSinonimos.length) + "</td>"
          + '<td><input type="text" id="txtSinonimoNombre" maxlength="100" class="table-input" value="" placeholder="ingrese sinónimo" /></td>'
          + "<td style='width:3%'><div class='accion-box'><a href='javascript:EliminarSinonimo(1)' title='Eliminar'><span class='fas fa-trash-alt'></span></a></div></td></tr>";
    $('#gridSinonimos').append(html);

    var sinonimo = "";
    sinonimo = {
        Sino_Id: listaSinonimos.length + 1,
        Sino_Nombre: ''
    };

    //listaSinonimos.push(sinonimo);
}
function EliminarPuerto(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar el Puerto "' + puerNombre + '"?', function (result) {
        if (result) {
            var idPuerto = { Puer_Id: puerId };
            
        } else {
            return false;
        }
    });
    $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
    //HidePopUp('divPopup');
}

function SeleccionarPuerto(puerId) {
    //ShowLoader();
    $('#idPuerto').html(puerId);

    var puertoSeleccionado = { PuerId_Id: puerId };
   
}


function ListarPuertos() {
}

function ValidarCamposPuerto() {
    var EsValidado = false;
    var mensaje = "";

    if ($('#txtNombre').val() == "") {
        mensaje += (mensaje == "" ? "" : "<br />") + "- Puerto"
    }

    if ($('#ddlPais').val() == 0 || $('#ddlPais').val() == '' || $('#ddlPais').val() == null) {
        mensaje += (mensaje == "" ? "" : "<br />") + "- País"
    }

    if (mensaje != "") {
        mensaje = "Por favor ingrese los datos en: <br />" + mensaje;
        alert(mensaje, "WARNING");
    } else {
        EsValidado = true;
    }
    return EsValidado;
}

function cargarPaís() {
    var tipo = 1;
}

function modificacion() {
    $('#puerto').html(1);
}
