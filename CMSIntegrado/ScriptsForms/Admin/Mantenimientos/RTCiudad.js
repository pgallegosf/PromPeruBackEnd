...
function InsertarPuerto() {
    ...
    alert('Se guardó la ciudad "' + nombre + '" correctamente', 'CHECK', function () {
        LimpiarPoppupPuerto();
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
        HidePopUp('divPopup');
    });
...
function EditarPuerto(puerId) {
    if (puerId != null && puerId != '') {
        ...
        $('#lblRegistro').html('Actualizar Ciudad');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
        //ShowPopUp('divPopup');
    }
}
...
function EliminarCarga(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar la ciudad "' + puerNombre + '"?', function (result) {
        if (result) {
