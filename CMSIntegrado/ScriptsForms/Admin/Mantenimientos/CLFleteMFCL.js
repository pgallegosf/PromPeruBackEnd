...
function InsertarPuerto() {
    ...
    alert('Se guardó el Flete Marítimo FCL "' + nombre + '" correctamente', 'CHECK', function () {
        LimpiarPoppupPuerto();
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
        HidePopUp('divPopup');
    });
...
function EliminarPuerto(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar el Flete Marítimo FCL "' + puerNombre + '"?', function (result) {
        if (result) {
