...
function LimpiarPoppupPuerto() {
    ...
    $('#lblRegistro').html('Crear Texto Sugerencia');
...
function EditarPuerto(puerId) {
    if (puerId != null && puerId != '') {
        ...
        $('#lblRegistro').html('Actualizar Texto Sugerencia');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
        //ShowPopUp('divPopup');
    }
}
...
function EliminarCarga(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar el texto de la sección sugerencia "' + puerNombre + '"?', function (result) {
        if (result) {
