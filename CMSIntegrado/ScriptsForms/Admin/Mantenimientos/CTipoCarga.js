...
function LimpiarPoppupPuerto() {
    ...
    $('#lblRegistro').html('Crear Tipo de Carga');
}
...
function EditarPuerto(puerId) {
    if (puerId != null && puerId != '') {
        ...
        $('#lblRegistro').html('Actualizar Tipo de Carga');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
		$('#txtNombre').focus();
        //ShowPopUp('divPopup');
    }
...
function EliminarPuerto(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar el tipo de carga "' + puerNombre + '"?', function (result) {
        if (result) {
