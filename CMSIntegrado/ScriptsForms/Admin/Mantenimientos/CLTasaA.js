...
function LimpiarPoppupPuerto() {
    ...
    $('#lblRegistro').html('Crear Tasa Arancelaria');
}
...
function InsertarPuerto() {
    ...
    alert('Se guardó la Tasa Arancelaria "' + nombre + '" correctamente', 'CHECK', function () {
        LimpiarPoppupPuerto();
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "0", opacity: "0" });
        HidePopUp('divPopup');
    });
...
function EditarPuerto(puerId) {
    if (puerId != null && puerId != '') {
        ...
        $('#lblRegistro').html('Actualizar Tasa Arancelaria');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
		$('#txtNombre').focus();
        //ShowPopUp('divPopup');
    }
...
function EliminarPuerto(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar la Tasa Arancelaria"' + puerNombre + '"?', function (result) {
        if (result) {
