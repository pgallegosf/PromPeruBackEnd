...
function LimpiarPoppupPuerto() {
    ...
    $('#lblRegistro').html('Crear Servicio');
}
...
function EditarSucursal(sucursalId) {
    if (sucursalId != null && sucursalId != '') {
        SeleccionarPuerto(sucursalId);
        $('#btnGuardar').html('<label>Actualizar</label>');
        $('#lblRegistro').html('Sucursal');
        $('.reveal-modal-bg').css({ transition: "all 0.4s linear", height: "100%", opacity: "1" });
		$('#txtNombre').focus();
        //ShowPopUp('divPopup');
    }
...
function EliminarPuerto(puerNombre, puerId) {
    confirm('¿Está seguro de eliminar el Servicio "' + puerNombre + '"?', function (result) {
        if (result) {
